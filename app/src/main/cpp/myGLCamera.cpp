...
MyGLCamera::MyGLCamera() {
    ...
    glm::vec3 cameraPosition = glm::vec3(xPosition, yPosition, zPosition);
...
void MyGLCamera::SetAspectRatio(float aspect) {
    ...
    projectionMat = glm::perspective(_fieldOfView * float(M_PI / 180), // camera's field-of-view
...
glm::mat4 MyGLCamera::GetMVP() {
    return mvpMat;
}

glm::mat4 MyGLCamera::GetMVP(glm::mat4& model) {
    return projectionViewMat * rotateMat * model;
}
